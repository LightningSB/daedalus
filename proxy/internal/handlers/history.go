...
// GetHistoryJSON serves JSON for DuckDB to read directly via read_json
func (h *HistoryHandler) GetHistoryJSON(w http.ResponseWriter, r *http.Request) {
	tgUserID := chi.URLParam(r, "tgUserId")
	if tgUserID == "" {
		http.Error(w, `{"error": "Missing user ID"}`, http.StatusBadRequest)
		return
	...
	data, err := parquet.GetHistoryAsJSON(r.Context(), h.minioClient, tgUserID)
	if err != nil {
		log.Printf("Error getting history JSON for user %s: %v", tgUserID, err)
...
// PostHistoryJSON saves history (same as PostHistory, for consistent naming)
func (h *HistoryHandler) PostHistoryJSON(w http.ResponseWriter, r *http.Request) {
	h.PostHistory(w, r)
}
